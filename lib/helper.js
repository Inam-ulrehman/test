...
export const formatGmailDate = (date) => {
  ...
  const gmailTime = moment(date) // Replace with your desired time

  let formattedTime

  if (currentTime.isSame(gmailTime, 'day')) {
    formattedTime = gmailTime.format('h:mm A')
  } else if (currentTime.isSame(gmailTime, 'year')) {
    formattedTime = gmailTime.format('MMM D')
  } else {
    formattedTime = gmailTime.format('MMM D, YYYY')
  }
