...
import { updateCategoriesThunk } from '@/features/products/categoriesSlice'
import { getStateValues } from '@/features/samples/samplesSlice'
import styled from 'styled-components'
const FormComponent = () => {
  const dispatch = useDispatch()
  const { message } = App.useApp()
  const { name, _id, editLoading } = useSelector((state) => state.categories)

  const onFinish = (value) => {
    const categories = { ...value, _id }
