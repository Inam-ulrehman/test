...
const ImageComponent = () => {
  ...
  const { _id, images } = useSelector((state) => state.subcategories)
  ...
  const props = {
    ...
    async onChange(info) {
      ...
      if (info.file.status === 'removed') {
        ...
        try {
          const result = await customFetch.post(
            '/authadmin/product/subcategory/destroyimage',
          ...
          dispatch(
            updateCategoriesThunk({
              subcategories: { images: removeImage, _id },
              message,
            })
          )
          // cloudinary response
      ...
      if (info.file.status === 'done') {
        ...
        const subcategories = { images: filterImages, _id }
        // update image in redux
